...
public final class Sprite extends Raster {
	...
	public void draw24BitSprite(int x, int y) {
		...
		if (!((width <= 0) || (height <= 0))) {
			set24BitPixels(width, height, Raster.pixels, raster, alpha, destOffset, srcOffset, destStep, srcStep);
		}
	}
	...
	public void drawTransparentSprite(int i, int j, int opacity) {
		...
		if (!(l1 <= 0 || k1 <= 0)) {
			method351(j1, l1, Raster.pixels, raster, j2, k1, i2, k, i1);
		}
	}
	...
	public void method346(int i, int j) {
		...
		if (k1 <= 0 || j1 <= 0) {
			...
			method347(l, k1, j1, i2, i1, l1, raster, Raster.pixels);
		}
	}
	...
	public void drawSprite(int i, int k) {
		...
		if (!(k1 <= 0 || j1 <= 0)) {
			method349(Raster.pixels, raster, i1, l, k1, j1, l1, i2); // draws sprites
		}
	}
	...
	public void method354(Background background, int i, int j) {
		...
		if (!(j1 <= 0 || i1 <= 0)) {
			method355(raster, j1, background.aByteArray1450, i1, Raster.pixels, 0, k1, k, l1, l);
		}
	}
	...
	public void drawARGBSprite(int xPos, int yPos, int alpha) {
		...
		if (!(spriteWidth <= 0 || spriteHeight <= 0)) {
			renderARGBPixels(spriteWidth, spriteHeight, raster, Raster.pixels, i1, alphaValue, j1, j2, i2);
		}
	}
