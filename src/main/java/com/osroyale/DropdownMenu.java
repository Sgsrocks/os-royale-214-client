...
public class DropdownMenu {
    ...
    private void drawDropBox(RSInterface child, int x, int y, boolean hover) {
        ...
        Raster.drawRectangle(x + 1, y + 1, width - 2, 22, 0);
        Raster.fillRectangle(x + 2, y + 2, width - 4, 20, bgColor);
    ...
    private void drawOpenBox(RSInterface child, int x, int y) {
        ...
        Raster.drawRectangle(x + width - 21, y + 3, 17, 17, 0);
        int len = options.length;
        if (len > 5) len = 5;
        Raster.fillRectangle(x + 1, y + 22, width - 2, SELECT_HEIGHT * len + 3, 0);
        ...
        for (int i = 0; i < len; i++) {
            ...
            Raster.fillRectangle(x + 2, y + yy + 22, width - 4 - (options.length > 5 ? 8 : 0), SELECT_HEIGHT, color);
        ...
        if (options.length > 5) {
            ...
            Raster.fillRectangle(x + width - 9, y + 24, 7, 13 * len, 0x534A3E);
            Raster.fillRectangle(x + width - 9, y + 25 + scrollPos, 7, scrollLength - 1, 0x777067);
        }
    }
    ...
    public void drawDropdown(RSInterface child, int x, int y) {
        ...
        if (open) {
            ...
            Raster.drawRectangle(x + width - 21, y + 4, 17, 17, 0);
        }
    }
