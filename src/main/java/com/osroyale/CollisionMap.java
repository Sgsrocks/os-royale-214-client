...
final class CollisionMap {
	...
	CollisionMap() {
		mapReionWidth = 104;
		mapRegionLength = 104;
		adjacencies = new int[mapReionWidth][mapRegionLength];
		method210();
	}
	...
	boolean withinDistanceWalls(int startX, int startY, int targetX, int targetY, int type, int direction) {
		...
		if (type == 1)
			if (direction == 0) {
				if (startX == targetX - 1 && startY == targetY)
					return true;
				if (startX == targetX && startY == targetY + 1 && (adjacencies[startX][startY] & 0x1280120) == 0)
					return true;
				if (startX == targetX && startY == targetY - 1 && (adjacencies[startX][startY] & 0x1280102) == 0)
					return true;
			} else if (direction == 1) {
				if (startX == targetX && startY == targetY + 1)
					return true;
				if (startX == targetX - 1 && startY == targetY && (adjacencies[startX][startY] & 0x1280108) == 0)
					return true;
				if (startX == targetX + 1 && startY == targetY && (adjacencies[startX][startY] & 0x1280180) == 0)
					return true;
			} else if (direction == 2) {
				if (startX == targetX + 1 && startY == targetY)
					return true;
		...
		if (type == 10) {
			if (startX == targetX && startY == targetY + 1 && (adjacencies[startX][startY] & WALL_SOUTH) == 0)
				return true;
			if (startX == targetX && startY == targetY - 1 && (adjacencies[startX][startY] & WALL_NORTH) == 0)
				return true;
			if (startX == targetX - 1 && startY == targetY && (adjacencies[startX][startY] & WALL_EAST) == 0)
				return true;
			if (startX == targetX + 1 && startY == targetY && (adjacencies[startX][startY] & WALL_WEST) == 0)
				return true;
		}
		return false;
	}
	...
	public boolean withinDistanceCorners(int startX, int startY, int targetX, int targetY, int type, int direction) {
		...
		if (type == 7 || type == 8) {
			...
			if (direction == 0) {
				if (targetX == startX + 1 && targetY == startY && (adjacencies[targetX][targetY] & WALL_WEST) == 0)
					return true;
				if (targetX == startX && targetY == startY - 1 && (adjacencies[targetX][targetY] & WALL_NORTH) == 0)
					return true;
			} else if (direction == 1) {
				if (targetX == startX - 1 && targetY == startY && (adjacencies[targetX][targetY] & WALL_EAST) == 0)
					return true;
				if (targetX == startX && targetY == startY - 1 && (adjacencies[targetX][targetY] & WALL_NORTH) == 0)
					return true;
			} else if (direction == 2) {
				if (targetX == startX - 1 && targetY == startY && (adjacencies[targetX][targetY] & WALL_EAST) == 0)
					return true;
				if (targetX == startX && targetY == startY + 1 && (adjacencies[targetX][targetY] & WALL_SOUTH) == 0)
					return true;
			} else if (direction == 3) {
				if (targetX == startX + 1 && targetY == startY && (adjacencies[targetX][targetY] & WALL_WEST) == 0)
					return true;
