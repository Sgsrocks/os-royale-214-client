...
public class RSFont extends Raster {
	...
	private void drawCharacter(byte charPixels[], int x, int y, int width, int height, int color) {
		int offset = x + y * Raster.width;
		int k1 = Raster.width - width;
		int l1 = 0;
		int pixel = 0;
		if (y < Raster.topY) {
			int offsetY = Raster.topY - y;
			height -= offsetY;
			y = Raster.topY;
			pixel += offsetY * width;
			offset += offsetY * Raster.width;
		}
		if (y + height >= Raster.bottomY) {
			height -= ((y + height) - Raster.bottomY);
		}
		if (x < Raster.topX) {
			int offsetX = Raster.topX - x;
			width -= offsetX;
			x = Raster.topX;
			pixel += offsetX;
			offset += offsetX;
		...
		if (!(width <= 0 || height <= 0)) {
			createPixels(Raster.pixels, charPixels, color, pixel, offset, width, height, k1, l1);
		}
	}
	...
	public void drawBaseStringMoveXY(String string, int drawX, int drawY, int[] xModifier, int[] yModifier) {
		...
		for (int currentCharacter = 0; currentCharacter < string.length(); currentCharacter++) {
			...
			if (strikethroughColor != -1) {
				rsDrawingArea.drawHorizontalLine(drawX, strikethroughColor, i_109_, drawY + (int) ((double) baseCharacterHeight * 0.7));
			}
			if (underlineColor != -1) {
				rsDrawingArea.drawHorizontalLine(drawX, underlineColor, i_109_, drawY + baseCharacterHeight);
			}
			drawX += i_109_;
		}
	}
	...
	public static void createCharacterPixels(int[] dest, byte[] src, int color, int srcPix, int destPix, int width, int height, int i_29_, int i_30_) {
		int i_31_ = -(width >> 2);
		width = -(width & 0x3);
		for (int h = -height; h < 0; h++) {
			for (int w = i_31_; w < 0; w++) {
				if (src[srcPix++] != 0) {
					dest[destPix++] = color;
				} else {
					destPix++;
	...
	public void drawCharacter(int character, int x, int y, int width, int height, int shadowColor) {
		...
		int i_41_ = Raster.width - width;
		int i_42_ = 0;
		int i_43_ = 0;
		if (y < Raster.topY) {
			int i_44_ = Raster.topY - y;
			height -= i_44_;
			y = Raster.topY;
			i_43_ += i_44_ * width;
			pixel += i_44_ * Raster.width;
		}
		if (y + height > Raster.bottomY) {
			height -= y + height - Raster.bottomY;
		}
		if (x < Raster.topX) {
			int i_45_ = Raster.topX - x;
			width -= i_45_;
			x = Raster.topX;
			i_43_ += i_45_;
			pixel += i_45_;
		...
		if (x + width > Raster.bottomX) {
			int i_46_ = x + width - Raster.bottomX;
			width -= i_46_;
			i_42_ += i_46_;
			i_41_ += i_46_;
		}
		if (width > 0 && height > 0) {
			createCharacterPixels(Raster.pixels, fontPixels[character], shadowColor, i_43_, pixel, width, height, i_41_, i_42_);
		}
	}
