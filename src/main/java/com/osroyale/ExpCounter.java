...
import com.osroyale.Client.ScreenMode;
...
public class ExpCounter {
	...
	static void drawExperienceCounter() {
		boolean isFixed = Client.frameMode == ScreenMode.FIXED;
		...
		if (Settings.COUNTER_POSITION == 0) {
			x = Client.frameWidth - boxWidth - 255;
		} else if (Settings.COUNTER_POSITION == 1) {
			x = (Client.frameWidth - boxWidth - (isFixed ? 255 : 0)) / 2;
...
//				return;
			...
			Raster.fillRectangle(x + 2, 32, width, 12, 0x37A351);
			...
			if (Client.instance.hover(x, y, Client.spriteCache.get(backgroundSprite))) {
				...
				Raster.drawTransparentBox(mouseX + 1, mouseY + 6, 113, level < 99 ? 78 : 48, 0x7A5E3D, 185);
