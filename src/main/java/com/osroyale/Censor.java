...
final class Censor {
	...
	public static void loadConfig(StreamLoader streamLoader) {
		Buffer stream = new Buffer(streamLoader.getFile("fragmentsenc.txt"));
		Buffer stream_1 = new Buffer(streamLoader.getFile("badenc.txt"));
		Buffer stream_2 = new Buffer(streamLoader.getFile("domainenc.txt"));
		Buffer stream_3 = new Buffer(streamLoader.getFile("tldlist.txt"));
	...
	private static void readFragmentsEnc(Buffer stream) {
		anIntArray620 = new int[stream.readUnsignedInt()];
